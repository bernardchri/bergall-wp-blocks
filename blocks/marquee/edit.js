import { useBlockProps, InspectorControls, InnerBlocks, URLInput } from '@wordpress/block-editor';
...
export default function Edit({ attributes, setAttributes }) {
    ...
    return (
        <div {...blockProps}>
            <InspectorControls>
                <div style={{ padding: '1rem' }}> {metadata.description}</div>
            ...
            <div
                ...
                style={{ overflow: 'scroll', transform: `rotate(${rotation}deg)` }}
            >
                <div
                    className={`marquee-content ${!previewMode ? '' : '--preview'} `}
                    style={{ '--speed': `${speed}s`, '--direction': direction ? 'forwards' : 'reverse' }}
                >
