...
export default function Edit( { attributes, setAttributes } ) {
	...
	useEffect( () => {
		setAttributes( {
			menuPrimaireHtml: findMenuToDisplay(
				allMenus,
				attributes.menuPrimaire
			),
			menuSecondaireHtml: findMenuToDisplay(
				allMenus,
				attributes.menuSecondaire
			),
			menuTertiaireHtml: findMenuToDisplay(
				allMenus,
				attributes.menuTertiaire
			),
		} );
	}, [
		allMenus,
			...
			<InspectorControls>
				<PanelBody title="Options">
					<MediaUpload
						onSelect={ ( media ) =>
							setAttributes( {
								logo: { ...attributes.logo, url: media.url },
							} )
						}
						allowedTypes={ [ 'image' ] }
						render={ ( { open } ) => (
					...
					<SelectControl
						...
						options={ menuOptions }
						onChange={ ( val ) =>
							setAttributes( { menuPrimaire: val } )
						}
					/>
					<SelectControl
						...
						options={ menuOptions }
						onChange={ ( val ) =>
							setAttributes( { menuSecondaire: val } )
						}
					/>
					<SelectControl
						...
						options={ menuOptions }
						onChange={ ( val ) =>
							setAttributes( { menuTertiaire: val } )
						}
					/>
				</PanelBody>
			</InspectorControls>
